...
const
   ...
   begin
      ...
      try
         ...
         case accessMode of
            fmOpenRead: raise Exception.Create('Missing read permissions: ' + path);
            fmOpenWrite: raise Exception.Create('Missing write permissions: ' + path);
            fmOpenReadWrite: raise Exception.Create('Missing read & write permissions: ' + path);
         end;
      end;
      CloseFile(LFile);
   end;
   ...
   begin
      if conn is TFDConnection then
         ...
         raise Exception.Create('Unrecognized delphi connection type: ' + conn.ClassName);

   end;

   procedure connectDB(conn: TObject; const iniSection, iniPath: string);
   begin
      var config: TConfig := nil;
